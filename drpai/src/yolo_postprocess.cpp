...
/* Soft-NMS: instead of hard-suppressing overlapping detections, reduce
 * their confidence based on overlap. This preserves weak but valid
 * detections that overlap with stronger ones (common for distant targets
 * where the model produces multiple uncertain candidates at slightly
 * different positions). Uses Gaussian penalty: score *= exp(-iou²/sigma). */
static std::vector<Detection> nms(std::vector<Detection>& candidates,
                                   float nms_thresh, int max_det) {
    const float sigma = 0.5f;  /* Gaussian decay sigma for soft-NMS */
    const float score_thresh = 0.05f;  /* discard below this after decay */
    ...
    for (size_t i = 0; i < candidates.size() && (int)results.size() < max_det; i++) {
        if (candidates[i].confidence < score_thresh) continue;

        results.push_back(candidates[i]);

        /* Soft-suppress remaining candidates that overlap */
        for (size_t j = i + 1; j < candidates.size(); j++) {
            if (candidates[i].class_id == candidates[j].class_id) {
                float overlap = iou(candidates[i], candidates[j]);
                if (overlap > 0.0f) {
                    /* Gaussian decay: high overlap → strong penalty */
                    candidates[j].confidence *= expf(-(overlap * overlap) / sigma);
                }
            }
        }
    }
    ...
    for (int s = 0; s < num_scales; s++) {
        ...
        for (int row = 0; row < grid_h; row++) {
            for (int col = 0; col < grid_w; col++) {
                ...
                float bx1 = (anchor_x - dist_left)   * stride;
                float by1 = (anchor_y - dist_top)    * stride;
                float bx2 = (anchor_x + dist_right)  * stride;
                float by2 = (anchor_y + dist_bottom) * stride;

                float bw = bx2 - bx1;
                float bh = by2 - by1;

                /* ── Minimal size filter: reject pixel noise only ── */
                /* All smart filtering (aspect ratio, expected size, etc.)
                   is done in Python where robot config and calibration
                   provide proper context. C++ only rejects clear noise. */
                if (bw < 4.0f || bh < 4.0f) continue;

...
#   3. Start the robot GUI with Camera + Navigation
#
# Usage:
...
[Unit]
Description=V2N Robot Control (Camera + Navigation)
...
ExecStart=/root/ros2_ws/src/target_nav/scripts/robot_start.sh
ExecStop=/root/ros2_ws/src/target_nav/scripts/robot_stop.sh
Restart=on-failure
RestartSec=10
...
do_start() {
    ...
    # Start bringup (LiDAR, Arduino, Odometry)
    echo "[1/2] Starting bringup (LiDAR, Arduino, Odometry)..."
    ros2 launch target_nav bringup.launch.py &
    BRINGUP_PID=$!
    sleep 5

    # Start main GUI
    echo "[2/2] Starting main GUI..."
    ros2 run target_nav main_gui
...
do_status() {
    ...
    pgrep -a -f "main_gui\|arduino_driver\|rplidar\|odometry_node" 2>/dev/null || echo "  No robot processes running"
    echo ""
